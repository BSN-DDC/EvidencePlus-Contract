...
contract Evidence is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // }
     ...
     function getRequestData(
        ...
        bool[] memory voter_status,
        bytes32[] memory did) {

        require(saveRequests[hash].hash == hash, "request not found");
        SaveRequest storage evidence = saveRequests[hash];

        address[] memory voter1 = new address[](evidence.voters.length);
        bool[] memory voter_status1 = new bool[](evidence.voters.length);
        bytes32[] memory did1 = new bytes32[](evidence.voters.length);

        // uint256 index = 0;
        for (uint i = 0; i < evidence.voters.length; i++) {
            voter1[i] = evidence.voters[i].voter;
            voter_status1[i] = evidence.voters[i].voter_status;
            did1[i] = evidence.voters[i].did;
    ...
    function voteSaveRequest(
        ...
        uint8 number = 0;
        for (uint i = 0; i < request.voters.length; i++) {
            if (request.voters[i].voter_status == status) {
                if (status == true) {
                    number++;
                    if (number >= threshold) {
                        voteStatus = 1;
    ...
    //查看存证数据
    function getEvidence(bytes32 hash) public view returns (
        bytes32,
        address owner,
        bytes memory remaks,
        uint256 timestamp,
        ...
        Evidence storage evidence = evidences[hash];

        require(evidences[hash].hash == hash, "Evidence not found");
        require(evidence.status == true, "The certificate data has been disabled");
